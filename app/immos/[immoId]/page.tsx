...
type ImmoDetailsPageProps = {
  params: Promise<{
    immoId: string;
  }>;
};

const DetailsImmoPage = ({ params }: ImmoDetailsPageProps) => {
  const { immoId } = use(params); // ✅ OBLIGATOIRE
  const numericImmoId = Number(immoId);
  ...
  useEffect(() => {
    ...
    const fetchImmo = async () => {
      const res = await fetch(`/api/immos/` + immoId, {
        cache: "no-store",
      });
      ...
      setClientId(data.immo.personId);

      /*       const immo: Immo = data.client.immos.find(
      ...
      setTask(data.immo.task);
      //setSalNet(1);
      /*     console.log("immo.salNet:", immo.salNet);
      console.log("immo.chqRep:", immo.chqRep);
      const sol = +immo.salNet.toString() + +immo.chqRep.toString();
      console.log("TOTAL: ", sol);
 */
      setMaritalStatus(data.immo.maritalStatus);
      setSalNet(+data.immo.salNet.toString());
      //  setSalNet(+data.immo.salNet as unknown as number);
      setSalNetCo(+data.immo.salNetCo.toString());
      setChqRep(+data.immo.chqRep.toString());
      setChqRepCo(+data.immo.chqRepCo.toString());
      setAutRev(+data.immo.autRev.toString());
      setAutRevCo(+data.immo.autRevCo.toString());
      setPrtTmp(+data.immo.prtTmp.toString());
      setPrtTmpCo(+data.immo.prtTmpCo.toString());
      setAutPrt(+data.immo.autPrt.toString());
      setAutPrtCo(+data.immo.autPrtCo.toString());

      setImmoStatus(data.immo.immoStatus as string);

      setAnaDone(data.immo.anaDone);
      setCahierCharge(
        data.immo?.cachierCharge ? data.immo?.cachierCharge : " "
      );

      setOfferDone(data.immo.offerDone);
      setOfferAccepted(data.immo.offerAccepted);
      setRechercheBien(
        data.immo?.rechercheBien ? data.immo?.rechercheBien : " "
      );

      setStartDate(data.immo?.startDate?.toString().split("T")[0]);
      setEndDate(data.immo?.endDate?.toString().split("T")[0]);
      setDemandeCours(data.immo?.demandeCours ? data.immo?.demandeCours : " ");
      setFileClosed(data.immo.fileClosed);

      setNotes(data.immo.notes as string);
    };
  ...
  return (
    <div className="w-full mx-auto ">
      <div className=" rounded-lg p-2 mt-2 bg-card">
...
export default DetailsImmoPage;
